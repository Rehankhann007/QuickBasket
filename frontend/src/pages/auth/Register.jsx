...
import {
  FiEye, FiEyeOff, FiUser, FiLock, FiMail, FiAtSign,
...
// ─── Reusable Input ──────────────────────────────────────────────────────────
const InputField = ({ icon: Icon, type = "text", placeholder, value, onChange, name, required, minLength }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-100 flex items-center justify-center px-4 py-8">
      ...
      >
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-3xl p-8 text-white text-center mb-4 shadow-xl shadow-blue-200">
          ...
          <p className="text-blue-100 mt-1 text-sm">Create your account to get started.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-100 p-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[
              { value: "user", label: "Customer", icon: FiShoppingCart },
              { value: "deliveryBoy", label: "Delivery Partner", icon: FiTruck },
            ].map(({ value, label, icon: Icon }) => (
              <button
                key={value}
                type="button"
                onClick={() => setForm({ ...form, role: value })}
                className={`flex flex-col items-center gap-2 py-4 rounded-2xl border-2 transition font-medium text-sm ${
                  form.role === value
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <InputField icon={FiUser} name="name" placeholder="Your full name" value={form.name} onChange={handleChange} required />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider pl-1">Username</label>
              <InputField icon={FiAtSign} name="username" placeholder="Choose a username" value={form.username} onChange={handleChange} required />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider pl-1">Email</label>
              <InputField icon={FiMail} type="email" name="email" placeholder="Your email address" value={form.email} onChange={handleChange} required />
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider pl-1">Password</label>
              <InputField icon={FiLock} type="password" name="password" placeholder="Min. 6 characters" value={form.password} onChange={handleChange} required minLength={6} />
            </div>
          ...
          <GoogleLoginButton role={form.role} onSuccess={handleGoogleSuccess} onError={setError} />
